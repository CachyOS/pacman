...
NetworkAccess = allowed (Optional)
--------
...
*NetworkAccess =* allowed::
	By default hooks are run inside a fresh network namespace with only a
	loopback interface and are aborted if that namespace cannot be created.
	Setting this option to 'allowed' runs the hook with full network access.
	Only use this for hooks that genuinely need the network (e.g. fetching
	mirror lists or keys).

...
*\--allow-net-prepare*::
	Run the prepare() function with network access. This is the default
	behaviour. Equivalent to adding 'netprepare' to BUILDENV. To isolate
	prepare() as well, set '!netprepare' in BUILDENV.
...
*\--nonetsandbox*::
	Do not isolate the build from the network. By default, when the
	'netsandbox' build environment option is enabled, everything that runs
	PKGBUILD code runs inside a network-isolated namespace,
	while sources are downloaded by makepkg itself beforehand. This option
	disables that isolation and runs the build with full network access.
	Equivalent to adding '!netsandbox' to BUILDENV in linkman:makepkg.conf[5].
	This is also required on systems that cannot create unprivileged
	user/network namespaces, and for builds that rely on `localhost`
	(127.0.0.1), as the isolated namespace has no usable loopback interface.

...
*\--disable-sandbox*::
	...
	`--disable-sandbox-filesystem`, `--disable-sandbox-syscalls` and
...
*\--disable-sandbox-network*::
	...
	Linux systems. Required in environments that cannot create network
	namespaces, such as unprivileged containers. See `DisableSandboxNetwork`
	in linkman:pacman.conf[5].

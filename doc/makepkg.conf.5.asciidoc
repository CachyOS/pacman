...
**OPTIONS=(**!strip docs libtool staticlibs emptydirs ...**)**::
	...
	*zipkmod*;;
		Compress manual kernel modules with zstd.
...
**KMOD_COMPRESSION_LEVEL=**"19"::

	If `zipkmod` is specified in the `OPTIONS` array, this variable will
	instruct makepkg which zstd compression level should be used to compress
	kernel modules. If not set, the default compression level is 19.

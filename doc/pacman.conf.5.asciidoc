...
*DisableSandbox*::
	...
	`DisableSandboxSyscalls` and `DisableSandboxNetwork`.
...
*DisableSandboxNetwork*::
	Disable the network isolation applied to package scriptlets and hooks on
	Linux systems. By default these are run inside a fresh network namespace
	containing only a loopback interface, and are aborted entirely if that
	namespace cannot be created (for example in unprivileged containers
	lacking `CAP_SYS_ADMIN`, or on kernels without network namespace
	support). Set this option in such environments to allow scriptlets and
	hooks to run again. Individual hooks that legitimately require network
	access can request it with the `NetworkAccess` option instead - see
	linkman:alpm-hooks[5].
